  ...
  useEffect(() => {
    if (initialData) {
      setFormData(prev => ({
        ...
        title: initialData.title || '',
        notes: 
          (initialData?.type === 'depositIdea' || (initialData as any)?.sourceDepositIdeaId)
            ? ''
            : (initialData?.notes || ''),
        amount: initialData?.amount?.toString() || prev.amount,
        withdrawalDate: initialData?.withdrawn_at ? new Date(initialData.withdrawn_at) : prev.withdrawalDate,
        dueDate: initialData?.due_date ? new Date(initialData.due_date) : prev.dueDate,
        time: initialData?.start_time ? timestampToTimeString(initialData.start_time) : prev.time,
        startTime: initialData?.start_time ? timestampToTimeString(initialData.start_time) : prev.startTime,
        endTime: initialData?.end_time ? timestampToTimeString(initialData.end_time) : prev.endTime,
        isAnytime: initialData?.is_all_day || false,
        is_urgent: initialData?.is_urgent || false,
        is_important: initialData?.is_important || false,
        is_authentic_deposit: initialData?.is_authentic_deposit || false,
        is_twelve_week_goal: initialData?.is_twelve_week_goal || false,
        countsTowardWeeklyProgress: (initialData as any)?.counts_toward_weekly_progress || false,
        inputKind: ((initialData as any)?.input_kind || 'count'),
        unit: ((initialData as any)?.unit || 'days'),
        schedulingType: (initialData?.type as any) || 'task',
        ...
        selectedGoalId: (initialData as any)?.goal_12wk_id || null,
          ...
          <View style={styles.weeklyPlanningGrid}>
            {cycleWeeks.map(week => (
              <View key={week.week_number} style={styles.weekCard}>
                ...
                <Text style={styles.weekDates}>{new Date(week.start_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</Text>
                <TextInput
                  style={styles.weekTargetInput}
                  value={(formData.weeklyTargets[week.week_number] ?? 0).toString()}
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.schedulingType === 'withdrawal') {
        ...
        const isGoalLinkedWithWeeklyPlan = !!formData.selectedGoalId && formData.is_twelve_week_goal && formData.countsTowardWeeklyProgress && !!currentCycle;

        if (isGoalLinkedWithWeeklyPlan && mode === 'create') {
          // Build weekly target selections
          const selectedWeeks = Object.entries(formData.weeklyTargets)
            .filter(([_, target]) => (target as number) > 0)
            .map(([weekNumber, target]) => ({
              weekNumber: parseInt(weekNumber, 10),
              targetDays: target as number,
            }));
          ...
          // Create task with week plan via hook
          const newTask = await createTaskWithWeekPlan({
            title: formData.title.trim(),
          ...
          if (!newTask) throw new Error('Failed to create goal‑linked task');

          // Add associations (roles/domains/KRs + note)
          ...
          if (roleJoins.length) joinPromises.push(supabase.from('0008-ap-universal-roles-join').insert(roleJoins));
          if (domainJoins.length) joinPromises.push(supabase.from('0008-ap-universal-domains-join').insert(domainJoins));
          if (krJoins.length) joinPromises.push(supabase.from('0008-ap-universal-key-relationships-join').insert(krJoins));
          await Promise.all(joinPromises);

        } else {
          // Standard create/update path
          const payload: any = {
            user_id: user.id,
            title: formData.title,
          ...
          const goalJoins = formData.selectedGoalId ? [{ parent_id: taskId, parent_type: 'task', goal_id: formData.selectedGoalId, user_id: user.id }] : [];
      ...
      <ScrollView style={styles.formContent}>
        ...
        {formData.schedulingType !== 'depositIdea' && formData.schedulingType !== 'withdrawal' && (
          <>
            ...
            {/* Added from 5.1: countsTowardWeeklyProgress + Input Kind + Unit */}
            {formData.is_twelve_week_goal && (
              <View style={{ marginBottom: 8 }}>
                <View style={styles.compactSwitchContainer}>
                  <Text style={styles.compactSwitchLabel}>Counts Toward Weekly Progress</Text>
                  <Switch value={formData.countsTowardWeeklyProgress} onValueChange={(val) => setFormData(prev => ({...prev, countsTowardWeeklyProgress: val}))} />
                </View>
                  ...
                  <View style={styles.inputKindSection}>
                    ...
                    <View style={styles.inputKindSelector}>
                      {(['count','duration'] as const).map(kind => (
                        <TouchableOpacity key={kind} style={[styles.inputKindButton, formData.inputKind === kind && styles.activeInputKindButton]} onPress={() => setFormData(prev => ({...prev, inputKind: kind}))}>
                          <Text style={[styles.inputKindButtonText, formData.inputKind === kind && styles.activeInputKindButtonText]}>{kind.charAt(0).toUpperCase()+kind.slice(1)}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                    ...
                    <View style={styles.unitSelector}>
                      {(['days','hours','sessions'] as const).map(unit => (
                        <TouchableOpacity key={unit} style={[styles.unitButton, formData.unit === unit && styles.activeUnitButton]} onPress={() => setFormData(prev => ({...prev, unit}))}>
                          <Text style={[styles.unitButtonText, formData.unit === unit && styles.activeUnitButtonText]}>{unit.charAt(0).toUpperCase()+unit.slice(1)}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  </View>
...
const styles = StyleSheet.create({
  ...
  compactSwitchContainer: { flexDirection: 'row', alignItems: 'center', gap: 8, justifyContent: 'space-between' },
  ...
  compactDateTimeRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 16, justifyContent: 'space-between' },
  ...
  // Weekly planning styles (added)
  sectionBox: { backgroundColor: '#f8fafc', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: '#e5e7eb', marginBottom: 12 },
  sectionHint: { fontSize: 12, color: '#6b7280', marginBottom: 8 },
  weeklyPlanningGrid: { flexDirection: 'row', gap: 12, paddingHorizontal: 4 },
  weekCard: { backgroundColor: '#ffffff', borderRadius: 8, padding: 12, alignItems: 'center', minWidth: 80, borderWidth: 1, borderColor: '#e5e7eb' },
  weekLabel: { fontSize: 12, fontWeight: '600', color: '#1f2937', marginBottom: 4 },
  weekDates: { fontSize: 10, color: '#6b7280', marginBottom: 8 },
  weekTargetInput: { backgroundColor: '#f8fafc', borderWidth: 1, borderColor: '#d1d5db', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 6, fontSize: 16, fontWeight: '600', color: '#1f2937', textAlign: 'center', width: 50, marginBottom: 4 },
  weekUnit: { fontSize: 10, color: '#6b7280', fontWeight: '500' },

  // Input kind styles (added)
  inputKindSection: { backgroundColor: '#f8fafc', borderRadius: 8, padding: 12, marginTop: 8 },
  subLabel: { fontSize: 14, fontWeight: '500', color: '#6b7280', marginBottom: 8, marginTop: 4 },
  inputKindSelector: { flexDirection: 'row', backgroundColor: '#ffffff', borderRadius: 6, padding: 2, marginBottom: 8 },
  inputKindButton: { flex: 1, paddingVertical: 6, paddingHorizontal: 12, borderRadius: 4, alignItems: 'center' },
  activeInputKindButton: { backgroundColor: '#0078d4' },
  inputKindButtonText: { fontSize: 12, fontWeight: '500', color: '#6b7280' },
  activeInputKindButtonText: { color: '#ffffff' },
  unitSelector: { flexDirection: 'row', backgroundColor: '#ffffff', borderRadius: 6, padding: 2 },
  unitButton: { flex: 1, paddingVertical: 6, paddingHorizontal: 8, borderRadius: 4, alignItems: 'center' },
  activeUnitButton: { backgroundColor: '#0078d4' },
  unitButtonText: { fontSize: 12, fontWeight: '500', color: '#6b7280' },
